...
import { BatteryCharging, Check, Gauge, Layers3, SlidersHorizontal, X } from "lucide-react";
  ...
  return (
    <section id="catalogo" className="catalog-section catalog-sheet-section">
      ...
      {filtered.length ? <div className="product-grid">{filtered.map((product) => (
        <button type="button" onClick={() => setSelected(product)} className="product-card" key={product.slug} aria-label={`Ver ${product.name}`}>
      ...
      {selected && <div className="product-modal-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}>
        <article className="product-modal" role="dialog" aria-modal="true" aria-label={`Información de ${selected.name}`}>
          ...
          <div className="modal-body"><div className="modal-visual" style={{ "--accent": selected.accent } as React.CSSProperties}><ProductVisual product={selected} large/></div><div className="modal-info"><div className="modal-specs"><div><Gauge/><strong>{selected.puffs.toLocaleString("es-PA")}</strong><span>Puffs</span></div><div><BatteryCharging/><strong>{selected.battery}</strong><span>Batería</span></div><div><Layers3/><strong>{selected.features[1]}</strong><span>Tecnología</span></div></div><ProductTabs slug={selected.slug} description={selected.description}/><div className="info-note">Consulta disponibilidad directamente en tu punto de venta.</div></div></div>
        </article>
      </div>}
    </section>
  );
